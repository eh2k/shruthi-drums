...
public:
    ...
    uint8_t getX() { return _settings.x;}
    uint8_t getY() { return _settings.y;}
    bool readDrumMatrix(uint8_t step, uint8_t instrument, uint8_t x, uint8_t y)
    {
        uint8_t level = readDrumMap(step, instrument, x, y);
        uint8_t threshold = ~_settings.density[instrument];
...
private:
    ...
    uint8_t _state;
    uint8_t _accentBits;
    uint8_t _pattern[kStepsPerPattern];
