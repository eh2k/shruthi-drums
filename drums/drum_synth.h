...
public:
	...
	static void LoadPatch(uint8_t instrument, uint8_t patch);

	static DrumPatch& Patch(uint8_t instrument)
	{
		return patch_[instrument];
	}

	static void Render(uint8_t cutoff)
	{
		Render(cutoff, AudioBuffer, 0xff);
	}

    static uint32_t idle_time_ms();

	static avrlib::RingBuffer<AudioBufferSpecs> CFBuffer;
    static avrlib::RingBuffer<AudioBufferSpecs> AudioBuffer;

private:
	static void Render(uint8_t cutoff, avrlib::RingBuffer<AudioBufferSpecs>& buffer, uint8_t instruments);
